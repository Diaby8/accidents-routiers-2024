...
"\n"
"#swapped / sign-flipped lat/long values: for each department, a genuine accident location\n"
"#should sit close to the department's median position. We try a set of plausible coordinate\n"
"#corruptions (swap lat/long, flip a sign, or both - e.g. a value typed as positive instead of\n"
"#negative) and keep whichever one lands closest to the row's own department median. This is\n"
"#data-driven (no hardcoded real-world coordinates), so it generalizes to any year of data.\n"
...
"\n"
"candidates = {\n"
"    'swap':          (coords['long_num'],  coords['lat_num']),\n"
"    'neg_lat':       (-coords['lat_num'],   coords['long_num']),\n"
"    'neg_long':      (coords['lat_num'],   -coords['long_num']),\n"
"    'swap_neg_lat':  (-coords['long_num'],  coords['lat_num']),\n"
"    'swap_neg_long': (coords['long_num'],  -coords['lat_num']),\n"
"}\n"
"best_dist = np.sqrt((coords['lat_num'] - dept_median['lat_num'])**2 + (coords['long_num'] - dept_median['long_num'])**2)\n"
"is_corrupted = pd.Series(False, index=coords.index)\n"
"for lat_try, long_try in candidates.values():\n"
"    dist_try = np.sqrt((lat_try - dept_median['lat_num'])**2 + (long_try - dept_median['long_num'])**2)\n"
"    is_corrupted |= (dist_try < best_dist) & (best_dist > 1.0) & (dist_try < 0.5)\n"
"print(f'Likely swapped or sign-flipped lat/long values: {is_corrupted.sum()}')"
))
...
"\n"
"#3b. fix swapped / sign-flipped lat/long values (detected in profiling: a row is corrupted\n"
"#    if a swap and/or a sign flip puts it much closer to its own department's median position)\n"
"has_coords = characteristics['lat'].notna() & characteristics['long'].notna()\n"
"idx = characteristics.loc[has_coords].index\n"
"dept_median = characteristics.loc[idx].groupby('dep')[['lat', 'long']].transform('median')\n"
"lat_, long_ = characteristics.loc[idx, 'lat'], characteristics.loc[idx, 'long']\n"
"candidates = {\n"
"    'swap':          (long_,  lat_),\n"
"    'neg_lat':       (-lat_,  long_),\n"
"    'neg_long':      (lat_,  -long_),\n"
...
"}\n"
"best_dist = np.sqrt((lat_ - dept_median['lat'])**2 + (long_ - dept_median['long'])**2)\n"
"best_lat, best_long = lat_.copy(), long_.copy()\n"
"for lat_try, long_try in candidates.values():\n"
"    dist_try = np.sqrt((lat_try - dept_median['lat'])**2 + (long_try - dept_median['long'])**2)\n"
...
"fixed_idx = idx[best_lat.ne(lat_) | best_long.ne(long_)]\n"
"characteristics.loc[idx, 'lat'] = best_lat\n"
"characteristics.loc[idx, 'long'] = best_long\n"
"print(f'Swapped/sign-flipped lat/long values fixed: {len(fixed_idx)}')\n"
"\n"
